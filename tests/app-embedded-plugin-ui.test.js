...
const mobileLayoutUi = fs.readFileSync(path.join(repoRoot, "public", "app-mobile-layout-ui.js"), "utf8");
...
assert.match(embeddedPluginUi, /const PLUGIN_HOST_MODEL_ESM_PATH = "\/vite-islands\/plugin-host-model\/plugin-host-model\.js"/);
assert.match(embeddedPluginUi, /function importPluginHostModel\(rootRef = \(typeof window !== "undefined" \? window : globalThis\)\)/);
assert.match(embeddedPluginUi, /__homeAiImportPluginHostModel/);
assert.match(embeddedPluginUi, /function currentPluginHostModel\(\)/);
assert.match(embeddedPluginUi, /currentPluginHostModel\(\)\?\.pluginManifestLaunchContextPlan/);
assert.match(embeddedPluginUi, /currentPluginHostModel\(\)\?\.pluginResidentShellContextPlan/);
...
assert.match(embeddedPluginUi, /function syncEmbeddedPluginIframeAuthCookie\(\)/);
assert.match(embeddedPluginUi, /window\.HomeAiRuntimeFacade\?\.auth\?\.syncCookie/);
assert.match(embeddedPluginUi, /document\.cookie = `hermes_web_key=\$\{encodeURIComponent\(key\)\}; Path=\/; SameSite=Lax\$\{secure\}`/);
assert.match(embeddedPluginUi, /syncEmbeddedPluginIframeAuthCookie\(\);\s*embeddedPluginHost\(def\)\.innerHTML = renderEmbeddedPluginFrame/);
...
assert.match(embeddedPluginUi, /embeddedPluginManifestMatchesLaunchContext\(record, embeddedPluginCurrentWorkspaceId\(\)\)/);
...
assert.match(embeddedPluginUi, /function embeddedPluginStableEntrySignature\(entryUrl = ""\)/);
assert.match(embeddedPluginUi, /function embeddedPluginEntryParamIsVolatile\(name = ""\)/);
assert.match(embeddedPluginUi, /codexpluginlaunch/);
assert.match(embeddedPluginUi, /function embeddedPluginFrameUsesEntry\(frame, entryUrl = ""\)/);
assert.match(embeddedPluginUi, /embeddedPluginFrameUsesEntry\(frame, record\.renderedEntryUrl\)/);
...
assert.match(embeddedPluginUi, /function scheduleEmbeddedPluginNavigationHealthCheck\(def, frame, observedAt = Date\.now\(\)\)/);
assert.match(embeddedPluginUi, /Number\(def\?\.navigationHealthTimeoutMs \|\| 8000\)/);
assert.match(embeddedPluginUi, /requestEmbeddedPluginHealthRefresh\(def, \{ force: true, reason: "navigation_health_timeout" \}\)/);
...
assert.match(embeddedPluginUi, /function embeddedPluginRawHostTopSafeArea\(\)/);
assert.match(embeddedPluginUi, /function embeddedPluginHostTopSafeArea\(def = embeddedPluginDefByView\(\)\)/);
assert.match(embeddedPluginUi, /stableHostTopSafeArea/);
assert.match(embeddedPluginUi, /Math\.abs\(raw - previous\) <= 2/);
assert.match(embeddedPluginUi, /safeAreaTop: hostTopSafeArea/);
assert.match(embeddedPluginUi, /hostTopSafeArea/);
assert.match(embeddedPluginUi, /function embeddedPluginViewportPayloadStableShape\(payload\)/);
assert.match(embeddedPluginUi, /function embeddedPluginViewportPayloadSignature\(payload\)/);
assert.match(embeddedPluginUi, /function embeddedPluginViewportPayloadSimilar\(left, right, tolerance = 2\)/);
assert.match(embeddedPluginUi, /function embeddedPluginViewportNoiseTolerance\(\)/);
assert.match(embeddedPluginUi, /contains\?\.\("native-shell-ios"\) \? 4 : 2/);
assert.match(embeddedPluginUi, /lastViewportPayloadSignature/);
assert.match(embeddedPluginUi, /lastViewportPayloadSnapshot/);
assert.match(embeddedPluginUi, /embeddedPluginViewportPayloadSimilar\(stableShape, record\.lastViewportPayloadSnapshot, embeddedPluginViewportNoiseTolerance\(\)\)/);
assert.match(embeddedPluginUi, /function sendEmbeddedPluginViewportMetrics\(def = embeddedPluginDefByView\(\), reason = "layout"\)/);
assert.match(embeddedPluginUi, /frame\.contentWindow\.postMessage\(payload, origin\)/);
...
assert.match(embeddedPluginUi, /if \(!pluginManifest && embeddedPluginResidentShellMatchesLaunchContext\(def, workspaceId, appearanceKey\)\)/);
assert.match(embeddedPluginUi, /embeddedPluginResidentShellRequiresFreshManifest\(def, record\.manifest\)[\s\S]*?loadEmbeddedPluginManifest\(def, \{ force: true \}\)\.catch\(showError\)/);
...
assert.match(embeddedPluginUi, /const loadedAt = Date\.now\(\);[\s\S]*?record\.frameLoadedAt = loadedAt/);
assert.match(embeddedPluginUi, /scheduleEmbeddedPluginNavigationHealthCheck\(def, frame, loadedAt\)/);
assert.match(embeddedPluginUi, /if \(!Number\(record\.navigationLastAt \|\| 0\)\) scheduleEmbeddedPluginNavigationHealthCheck\(def, frame, Date\.now\(\)\)/);
assert.match(embeddedPluginUi, /if \(Number\(record\.navigationLastAt \|\| 0\) > 0\) return;/);
...
assert.match(embeddedPluginUi, /sandbox="allow-same-origin allow-scripts allow-forms allow-downloads allow-modals allow-popups allow-popups-to-escape-sandbox"/);
assert.match(mobileLayoutUi, /scheduleEmbeddedPluginViewportBroadcast\(active \? "host_keyboard" : "host_keyboard_clear", 0\)/);
assert.match(mobileLayoutUi, /scheduleEmbeddedPluginViewportBroadcast\("plugin_context_viewport", 0\)/);
assert.doesNotMatch(composerContextUi, /scheduleEmbeddedPluginViewportBroadcast\(active \? "host_keyboard" : "host_keyboard_clear", 0\)/);
...
assert.match(embeddedPluginUi, /const workspaceId = embeddedPluginCurrentWorkspaceId\(\);[\s\S]*?const params = new URLSearchParams\(\{ workspaceId \}\);/);
...
assert.match(embeddedPluginUi, /const selected = String\(state\.selectedWorkspaceId \|\| ""\)\.trim\(\)/);
assert.match(embeddedPluginUi, /const authWorkspaceId = String\(state\.auth\?\.workspaceId \|\| ""\)\.trim\(\)/);
assert.match(embeddedPluginUi, /if \(!state\.auth\?\.isOwner && authWorkspaceId\) \{/);
assert.match(embeddedPluginUi, /if \(!selected \|\| selected === "owner" \|\| !allowed\.has\(selected\)\) return authWorkspaceId/);
...
assert.match(automationUi, /codex: state\.viewMode === "codex"/);
assert.match(automationUi, /finance: state\.viewMode === "finance"/);
assert.match(automationUi, /email: state\.viewMode === "email"/);
assert.match(automationUi, /growth: state\.viewMode === "growth"/);
assert.match(automationUi, /moira: state\.viewMode === "moira"/);
...
assert.match(wireStartUi, /openPinnedPluginBottomTab\("codex", typeof rememberCodexPluginReturnRoute === "function" \? rememberCodexPluginReturnRoute : null\)/);
assert.match(wireStartUi, /openPinnedPluginBottomTab\("finance", typeof rememberFinancePluginReturnRoute === "function" \? rememberFinancePluginReturnRoute : null\)/);
assert.match(wireStartUi, /openPinnedPluginBottomTab\("email", typeof rememberEmailPluginReturnRoute === "function" \? rememberEmailPluginReturnRoute : null\)/);
assert.match(wireStartUi, /openPinnedPluginBottomTab\("growth", typeof rememberGrowthPluginReturnRoute === "function" \? rememberGrowthPluginReturnRoute : null\)/);
assert.match(wireStartUi, /openPinnedPluginBottomTab\("moira", typeof rememberMoiraPluginReturnRoute === "function" \? rememberMoiraPluginReturnRoute : null\)/);
...
assert.match(embeddedPluginUi, /const keepBottomButton = embeddedPluginBottomButtonShouldStayVisible\(def\)/);
assert.match(embeddedPluginUi, /button\.hidden = !keepBottomButton/);
...
assert.match(navigationSearchUi, /setBottomTabHidden\(node, hiddenBottomTabs\.has\(id\) && !pinnedBottomTabButtonIds\.has\(id\)\)/);
...
const threadMessageUi = fs.readFileSync(path.join(repoRoot, "public", "app-thread-message-ui.js"), "utf8");
assert.match(threadMessageUi, /plan\?\.shouldClearKeyboardViewportMetrics \?\? !enabled/);
assert.match(threadMessageUi, /typeof clearKeyboardViewportMetrics === "function"\) clearKeyboardViewportMetrics\(\);/);
...
assert.match(stylesCss, /\.global-plugin-dock-mode \.topic-plugin-dock\s*\{[\s\S]*position: fixed;[\s\S]*bottom: calc\(var\(--topic-plugin-dock-bottom\) - var\(--topic-plugin-dock-nav-overlap\) \+ var\(--topic-plugin-dock-collapsed-safe-lift\)\);/);
