...
const appMobileLayoutJs = fs.readFileSync(path.join(repoRoot, "public", "app-mobile-layout-ui.js"), "utf8");
const appComposerContextJs = fs.readFileSync(path.join(repoRoot, "public", "app-composer-context-ui.js"), "utf8");
const appComposerDraftJs = fs.readFileSync(path.join(repoRoot, "public", "app-composer-draft-ui.js"), "utf8");
...
assert.match(appMobileLayoutJs, /function mobileBottomCssPx\(name, defaultValue = 0\)/);
assert.match(appMobileLayoutJs, /function captureClientLayoutDiagnostic\(reason = "layout"\)/);
assert.match(appMobileLayoutJs, /fetch\("\/api\/client-layout-diagnostics"/);
assert.match(appMobileLayoutJs, /const CLIENT_LAYOUT_DIAGNOSTIC_DEFAULT_MAX_PER_SESSION = 8/);
assert.match(appMobileLayoutJs, /const CLIENT_LAYOUT_DIAGNOSTIC_DEFAULT_MIN_INTERVAL_MS = 30000/);
assert.match(appMobileLayoutJs, /function clientLayoutDiagnosticsVerboseEnabled\(\)/);
assert.match(appMobileLayoutJs, /params\.get\("layoutDebug"\) === "1"/);
assert.match(appMobileLayoutJs, /localStorage\.getItem\("hermesClientLayoutDiagnostics"\) === "1"/);
assert.match(appMobileLayoutJs, /function clientLayoutDiagnosticShouldSend\(reason = "layout"\)/);
assert.match(appMobileLayoutJs, /state\.clientLayoutDiagnosticSentCount = sent \+ 1/);
assert.match(appMobileLayoutJs, /if \(!clientLayoutDiagnosticShouldSend\(reason\)\) return null;/);
assert.match(appMobileLayoutJs, /function scheduleClientLayoutDiagnostics\(reason = "layout", delays = \[0, 240, 900, 1800\]\) \{[\s\S]*?if \(!clientLayoutDiagnosticBudgetAvailable\(\)\) return;/);
assert.match(appMobileLayoutJs, /function clientLayoutDiagnosticViewportUnits\(\)/);
assert.match(appMobileLayoutJs, /function clientLayoutDiagnosticSafeAreaProbe\(\)/);
assert.match(appMobileLayoutJs, /function clientLayoutDiagnosticChrome\(app, main, bottomNav\)/);
assert.match(appMobileLayoutJs, /statusBarStyle: statusMeta\?\.getAttribute\("content"\) \|\| ""/);
assert.match(appMobileLayoutJs, /viewportUnits: clientLayoutDiagnosticViewportUnits\(\)/);
assert.match(appMobileLayoutJs, /rootElement: clientLayoutDiagnosticRect\(document\.documentElement\)/);
assert.match(appMobileLayoutJs, /function settleMobileBottomNavReservation\(reason = "layout", delays = \[0, 40, 120, 260, 520, 1000, 1800\]\)/);
assert.match(appMobileLayoutJs, /requestAnimationFrame\(\(\) => \{[\s\S]*?requestAnimationFrame\(\(\) => \{[\s\S]*?updateMobileBottomNavReservation\(\);/);
assert.match(appMobileLayoutJs, /window\.__hermesMobileBottomLayoutLastSettle = \{/);
assert.match(appMobileLayoutJs, /settleMobileBottomNavReservation\("view_change", \[0, 80, 240, 520\]\)/);
assert.doesNotMatch(appComposerContextJs, /function updateMobileBottomNavReservation\(\)/);
assert.doesNotMatch(appComposerContextJs, /function captureClientLayoutDiagnostic\(reason = "layout"\)/);
assert.doesNotMatch(appComposerContextJs, /function settleMobileBottomNavReservation\(reason = "layout"/);
assert.match(appPlatformUiJs, /settleMobileBottomNavReservation\("app_show"\)/);
assert.doesNotMatch(appChatComposerUiJs, /settleMobileBottomNavReservation\("host_foreground"/);
assert.match(appComposerDraftJs, /settleMobileBottomNavReservation\("host_foreground", \[0, 80, 240, 520, 1000\]\)/);
assert.match(appMobileLayoutJs, /const navVisualLift = Math\.max\(0, Math\.ceil\(mobileBottomCssPx\("--mobile-bottom-nav-visual-lift", 0\)\)\)/);
assert.match(appMobileLayoutJs, /const visualViewportHeight = Math\.ceil\(window\.visualViewport\?\.height \|\| 0\)/);
assert.match(appMobileLayoutJs, /const innerHeight = Math\.ceil\(window\.innerHeight \|\| 0\)/);
assert.match(appMobileLayoutJs, /const documentHeight = Math\.ceil\(document\.documentElement\?\.clientHeight \|\| 0\)/);
assert.match(appMobileLayoutJs, /const layoutViewportHeight = Math\.max\(innerHeight, documentHeight, visualViewportHeight\)/);
assert.match(appMobileLayoutJs, /const viewportHeight = layoutViewportHeight/);
assert.match(appMobileLayoutJs, /const comfortInset = Math\.max\(0, Math\.ceil\(mobileBottomCssPx\("--mobile-bottom-nav-comfort-inset", 0\)\)\)/);
assert.match(appMobileLayoutJs, /const navLaidOut = Boolean\(rect && rectHeight > 0 && rectWidth > 0 && rect\.bottom > 0\)/);
assert.match(appMobileLayoutJs, /const navBottomOverflowRaw = navLaidOut && viewportHeight \? Math\.ceil\(Math\.max\(0, rect\.bottom - viewportHeight\)\) : 0/);
assert.match(appMobileLayoutJs, /const navBottomOverflowClamp = Math\.max\(0, Math\.ceil\(mobileBottomCssPx\("--mobile-bottom-nav-overflow-clamp", 0\)\)\)/);
assert.match(appMobileLayoutJs, /const navBottomOverflow = Math\.min\(navBottomOverflowRaw, navBottomOverflowClamp\)/);
assert.match(appMobileLayoutJs, /const currentNavBottomDrop = navLaidOut \? Math\.max\(0, -currentNavBottom\) : 0/);
assert.match(appMobileLayoutJs, /const navBottomGapRaw = navLaidOut && viewportHeight \? Math\.ceil\(Math\.max\(0, viewportHeight - rect\.bottom \+ currentNavBottomDrop\)\) : 0/);
assert.match(appMobileLayoutJs, /const navBottomUnderflowRaw = Math\.max\(0, navBottomGapRaw - comfortInset\)/);
assert.match(appMobileLayoutJs, /const navBottomUnderflowClamp = Math\.max\(0, Math\.ceil\(mobileBottomCssPx\("--mobile-bottom-nav-underflow-clamp", 0\)\)\)/);
assert.match(appMobileLayoutJs, /const navBottomUnderflow = Math\.min\(navBottomUnderflowRaw, navBottomUnderflowClamp\)/);
assert.match(appMobileLayoutJs, /const largeViewportHeight = Math\.ceil\(clientLayoutDiagnosticMeasureLength\("100lvh"\)\?\.height \|\| 0\)/);
assert.match(appMobileLayoutJs, /const surfaceViewportHeight = standaloneSurface \? Math\.max\(viewportHeight, largeViewportHeight\) : viewportHeight/);
assert.match(appMobileLayoutJs, /const surfaceUnderflowSafeClamp = safeAreaTop > 0 \? Math\.min\(surfaceUnderflowClamp, safeAreaTop\) : 0/);
assert.match(appMobileLayoutJs, /const surfaceUnderflowCandidate = Math\.min\(surfaceUnderflowRaw, surfaceUnderflowSafeClamp\)/);
assert.match(appMobileLayoutJs, /const surfaceUnderflow = 0/);
assert.match(appMobileLayoutJs, /const effectiveNavBottomUnderflow = navBottomUnderflow/);
assert.match(appMobileLayoutJs, /const navBottom = navBottomOverflow \+ comfortInset - effectiveNavBottomUnderflow/);
...
assert.match(appMobileLayoutJs, /const dockExpanded = Boolean\(dockVisible && dock\?\.classList\.contains\("global-plugin-dock-expanded"\)\)/);
assert.match(appMobileLayoutJs, /const dockCollapsedHeight = Math\.max\(0, Math\.ceil\(mobileBottomCssPx\("--topic-plugin-dock-collapsed-height", 30\)\)\)/);
...
assert.match(appMobileLayoutJs, /const viewportOverflowRaw = Math\.max\(0, appHeight - viewportHeight\)/);
