...
`plan-plugin-workspace-provisioning`, and
`data/plugin-workspace-provisioning-apply.json` from
`apply-plugin-workspace-provisioning`. The plan must be schema version 1,
generated by the plan phase, include the ordinary default business plugins for
every planned workspace, and explicitly state that it does not create plugin
keys, workspace grants, or plugin bind/register calls. The apply report must
then prove that local plugin keys/config, workspace authorization grants,
plugin bind/register calls, and Gateway MCP materialization ran without
...
- Historical NAS/Windows migrations once treated the Owner Wardrobe binding as
  a special migrated-data case. That is no longer the fresh-install contract:
  Wardrobe is provisioned through the same workspace-local plugin contract as
  other ordinary private plugins. A clean Home AI install may create
  `wardrobe:owner` for Owner and should not skip Wardrobe binding validation
  merely because the workspace is Owner. Existing installations that still carry
  an older migrated Wardrobe identity should migrate that identity explicitly
  before running the standard provisioning apply phase.
- For a clean public/NAS install with no migrated plugin data, Owner must still
  ...
  plugin matrix explicit, and `apply-plugin-workspace-provisioning` then
  performs the bounded first-install registration/authorization/Gateway
  materialization before first-start preflight.
