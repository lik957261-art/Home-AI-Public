...
assert.match(embeddedPluginUi, /routeView === "growth" && \(pluginRoute \|\| pluginItemId\)/);
...
assert.match(stylesCss, /\.codex-mode \.conversation,[\s\S]*?\.email-mode \.conversation,[\s\S]*?\.health-mode \.conversation,[\s\S]*?\.note-mode \.conversation,[\s\S]*?\.growth-plugin-mode \.conversation\s*\{[\s\S]*padding: 0;/);
...
assert.match(stylesCss, /\.plugin-context-nav-mode #bottomWardrobeMode,[\s\S]*#bottomFinanceMode,[\s\S]*#bottomEmailMode,[\s\S]*#bottomHealthMode,[\s\S]*#bottomNoteMode,[\s\S]*#bottomGrowthMode\s*\{[\s\S]*order: 2;/);
