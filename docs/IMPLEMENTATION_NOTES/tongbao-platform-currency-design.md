...
兑换桥不得重新扫描 Growth 卡片状态来判断可兑换额度。卡片完成时已经把奖励结算成 Growth 金币；月结时的事实来源是 Growth 金币余额/流水。对于插件化 Growth，平台兑换桥应先读取插件的学习金币余额，再在同一受控操作中写入平台 `通宝` 流水并调用插件的幂等月结清零/扣减能力。清零/扣减记录必须以 idempotency key 去重，重复请求不得重复扣减 Growth 金币或重复入账 `通宝`。
