...
## English Writing Feedback Contract

English writing cards are a two-stage learning loop, not a one-click completion flow:

1. First submission is `writing_draft`.
   - Save the raw answer only as the Kanban comment.
   - Return structured draft feedback with strengths, focus areas, a rewrite checklist, and reflection prompts.
   - Generate a Markdown feedback report.
   - Do not complete the card and do not settle coins.
2. Second submission is `writing_revision`.
   - Evaluate whether the rewrite responded to the draft feedback.
   - Produce a final conclusion, next-practice guidance, and a Markdown deliverable report.
   - Only the service layer may complete the card and settle rewards.

The visible feedback must be instructional enough for the learner to know what to change next. A score alone is not a valid Growth writing evaluation.
