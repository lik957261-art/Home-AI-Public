...
the installer creates `.hermes-email`, grants `hermes-host` narrow search ACLs
on private parent directories plus a write ACL only on `.hermes-email`, then
reasserts workspace user ownership while keeping only the bounded service ACL
needed for future refresh.
