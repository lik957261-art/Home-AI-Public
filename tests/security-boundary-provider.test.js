...
function run() {
  ...
  const protectedRoot = "/Users/example/protected-source";
  const exceptionRoot = "/Users/example/workspace-drive";
  const protectedSecret = "/Users/example/protected-source/.hermes_web_secret_key";
  ...
  assert.strictEqual(provider.rootConflictsWithProtected(protectedRoot), true);
  assert.strictEqual(provider.rootConflictsWithProtected(exceptionRoot), false);
  ...
  const policy = provider.hardenAccessPolicy({
    ...
    default_workspace: protectedRoot,
    allowed_roots: [
      protectedRoot,
      exceptionRoot,
      "/home/example/.hermes/run-logs",
  ...
  assert.deepStrictEqual(policy.allowed_roots, [exceptionRoot, "/home/example/.hermes/run-logs"]);
  ...
  assert.match(permissionInstructions, /PowerPoint PPTX\/PPTM and Excel XLSX\/XLSM text extraction inside the current allowed roots is ordinary low-permission file analysis/);
  ...
  assert.match(permissionInstructions, /`pdf_extract_text`/);
  assert.match(permissionInstructions, /`pdf_render_pages`/);
  ...
  assert.match(permissionInstructions, /ZIP archive listing and safe extraction .* ordinary low-permission file analysis/);
  assert.match(permissionInstructions, /`archive_list`/);
  assert.match(permissionInstructions, /`archive_extract_safe`/);
  ...
  assert.match(fs.readFileSync(skillPath, "utf8"), /PowerPoint PPTX\/PPTM and Excel XLSX\/XLSM text extraction inside the current run's allowed roots is \*\*Allowed\*\*/);
  ...
  assert.match(fs.readFileSync(skillPath, "utf8"), /Use `pdf_extract_text` first/);
  assert.match(fs.readFileSync(skillPath, "utf8"), /use `pdf_render_pages` and then send the rendered page images to vision\/OCR/);
  assert.match(fs.readFileSync(skillPath, "utf8"), /Do not ask the user to export PDF pages manually/);
  ...
  assert.match(fs.readFileSync(skillPath, "utf8"), /ZIP archive listing and safe extraction inside the current run's allowed roots is \*\*Allowed\*\*/);
  assert.match(fs.readFileSync(skillPath, "utf8"), /Use `archive_list` and `archive_extract_safe`/);
  assert.match(fs.readFileSync(skillPath, "utf8"), /OCR, document-image extraction, and visual analysis of image files inside the current run's allowed roots are \*\*Allowed\*\*/);
  assert.match(fs.readFileSync(skillPath, "utf8"), /PDF containers must first go through `pdf_extract_text` or `pdf_render_pages`/);
