...
assert.match(runnerText, /path\.join\(\s*DEFAULT_HOME_AI_ROOT,\s*"plugins",\s*"codex-mobile-web",\s*"scripts",\s*"create-thread-task-card\.js"/);
assert.match(runnerText, /path\.dirname\(APP_ROOT\)[\s\S]*?"codex-mobile-web"[\s\S]*?"create-thread-task-card\.js"/);
assert.doesNotMatch(runnerText, /\/Users\/(?:hermes-host|example)\//);
