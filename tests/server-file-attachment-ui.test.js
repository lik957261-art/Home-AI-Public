...
assert.match(uploadSidebarUi, /UPLOAD_SIDEBAR_MODEL_ESM_PATH/);
assert.match(uploadSidebarUi, /\/vite-islands\/upload-sidebar-model\/upload-sidebar-model\.js/);
assert.match(uploadSidebarUi, /function importUploadSidebarModel\(/);
assert.match(uploadSidebarUi, /function currentUploadSidebarModel\(/);
assert.match(uploadSidebarUi, /attachFileMenuPlan/);
assert.match(uploadSidebarUi, /nativeShareIntakePanelPlan/);
assert.match(uploadSidebarUi, /serverFileAttachmentRequestPlan/);
...
assert.match(uploadSidebarUi, /label \|\| "系统文件"/);
assert.match(uploadSidebarUi, /data-attach-menu-server/);
assert.match(uploadSidebarUi, /label \|\| "服务器文件"/);
...
assert.match(uploadSidebarUi, /state\.directoryPath = directoryPlan\?\.directoryPath \|\| systemShareDirectoryPath\(\)/);
...
assert.match(uploadSidebarUi, /<button type="button" class="pending-artifact-remove" data-remove-artifact="\$\{index\}" aria-label="\$\{escapeHtml\(`移除 \$\{name\}`\)\}"><\/button>/);
assert.match(uploadSidebarUi, /data-pending-artifact-preview/);
assert.match(uploadSidebarUi, /openImagePreviewOverlay/);
assert.match(uploadSidebarUi, /const canAttachServerFile = serverFileAttachmentOwnerOnly\(\)/);
assert.match(uploadSidebarUi, /const attachLabel = panelPlan\?\.attachLabel \|\| \(canAttachServerFile \? "附加到当前对话" : "服务器文件附加仅限 Owner"\)/);
...
assert.match(uploadSidebarUi, /panelPlan\?\.attachButtonLabel \|\| \(canAttachServerFile \? "附加" : "Owner专用"\)/);
assert.match(uploadSidebarUi, /data-native-share-open-directory title="打开文件目录" aria-label="打开文件目录">\$\{escapeHtml\(panelPlan\?\.directoryLabel \|\| "目录"\)\}<\/button>/);
assert.match(uploadSidebarUi, /data-native-share-clear title="仅保存，不附加" aria-label="仅保存，不附加">\$\{escapeHtml\(panelPlan\?\.clearLabel \|\| "保存"\)\}<\/button>/);
...
assert.match(styles, /\.pending-artifact-preview img \{[\s\S]*?object-fit: contain;[\s\S]*?image-orientation: from-image;/);
