...
- X Search is **Allowed** when the run has the `x_search` toolset and the Gateway profile already has xAI OAuth/API credentials. Do not request Owner elevation just to search public X content.
- Scoped HTTP/API requests to the current account/workspace's documented Program APIs are **Allowed** when the run has the `http` toolset. Use `http_request` for authenticated manifest, bundle, validation, writeback, or same-workspace image upload calls whose endpoint, credential, and scope are documented in an allowed workspace file; do not look for `web_request`. For file uploads, pass in-scope local image bytes through `file_body` or `multipart_files`; do not send local path strings or `file://` URLs as if they were uploaded attachments. Do not use terminal/code for these calls unless those developer toolsets are explicitly allowed.
...
- Deleting an empty in-scope directory is **Allowed** when the run has the `file` toolset. Deleting a non-empty directory, recursive directory removal, or broad cleanup of a directory tree is **Needs elevation** for restricted Owner runs and must use the Owner approval marker before any tool call.
- Word DOCX generation inside the current run's allowed roots is **Allowed** when the run has the `file` toolset. Use `docx_create` for explicit real .docx document requests, write only inside allowed roots, and return the generated `MEDIA:<path>` line.
...
- PowerPoint PPTX/PPTM and Excel XLSX/XLSM text extraction inside the current run's allowed roots is **Allowed** when the run has the `file` toolset. Use `office_extract_text` when `read_file` cannot decode an Office document package.
- PowerPoint PPTX generation inside the current run's allowed roots is **Allowed** when the run has the `file` toolset. Use `pptx_create` for explicit real .pptx deck requests, write only inside allowed roots, and return the generated `MEDIA:<path>` line only after compatibility validation succeeds. Use `pptx_validate` to re-check existing in-scope decks when PowerPoint compatibility is in doubt.
- PDF generation inside the current run's allowed roots is **Allowed** when the run has the `file` toolset. Use `pdf_create` for explicit real .pdf document requests, write only inside allowed roots, and return the generated `MEDIA:<path>` line.
- PDF text extraction and PDF-to-page-image rendering inside the current run's allowed roots are **Allowed** when the run has the `file` toolset. Use `pdf_extract_text` first; if the PDF has no text layer or text is empty, use `pdf_render_pages` and then send the rendered page images to vision/OCR. Do not ask the user to export PDF pages manually.
- Audio transcription for in-scope MP3/M4A/WAV/AAC/OGG/OPUS/AMR/FLAC files is **Allowed** when the run has the `file` toolset. Use `audio_transcribe` for voice notes, reading retellings, and other audio-only files; do not ask the user to convert audio to video or route MP3 files through `video_analyze`.
- ZIP archive listing and safe extraction inside the current run's allowed roots is **Allowed** when the run has the `file` toolset. Use `archive_list` and `archive_extract_safe` instead of requesting shell, terminal, package installation, or manual user extraction just to unzip an in-scope archive.
- OCR, document-image extraction, and visual analysis of image files inside the current run's allowed roots are **Allowed** when the run has the `vision` toolset. PDF containers must first go through `pdf_extract_text` or `pdf_render_pages` when those file tools are available.
...
- Owner low Gateway runs may read, create, and update Owner Skills when the run has the `skills` toolset. Non-Owner workspace runs may read shared linked Skills. Even if a shared linked Skill path is inside `allowed_roots` and is filesystem-writable, Non-Owner runs must not directly create, update, delete, install, publish, or write-test shared Skills, Owner Skills, or Owner full Skill stores; those operations are **Needs elevation** unless routed through a creator/Owner-approved Hermes Mobile Skill write path.
