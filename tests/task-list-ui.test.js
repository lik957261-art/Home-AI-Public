...
const CLIENT_VERSION = "20260603-directory-topic-back-v511";
const appJs = [
...
assert.match(indexHtml, /<link rel="preload" href="\/styles\.css\?v=20260603-directory-topic-back-v511" as="style" onload="this\.onload=null;this\.rel='stylesheet'">/);
assert.match(indexHtml, /<noscript><link rel="stylesheet" href="\/styles\.css\?v=20260603-directory-topic-back-v511"><\/noscript>/);
...
assert.match(stylesCss, /\.wardrobe-mode \.composer,[\s\S]*?\.finance-mode \.composer,[\s\S]*?\.email-mode \.composer,[\s\S]*?\.health-mode \.composer,[\s\S]*?\.codex-mode \.composer \{[\s\S]*?display: none;/);
...
assert.match(indexHtml, /id="bottomChatMode"[\s\S]*id="bottomInboxMode"[\s\S]*id="bottomTasksMode"[\s\S]*id="bottomProjectsMode"[\s\S]*id="bottomTodosMode"[\s\S]*id="bottomWardrobeMode"[\s\S]*id="bottomCodexMode"[\s\S]*id="bottomFinanceMode"[\s\S]*id="bottomEmailMode"[\s\S]*id="bottomHealthMode"/);
...
assert.match(appJs, /"bottomEmailMode", "bottomHealthMode", "bottomLearningMode", "bottomAutomationMode"/);
...
assert.match(stylesCss, /\.app\.plugin-context-nav-mode\.wardrobe-plugin-host-active \{[\s\S]*?padding-bottom: var\(--plugin-context-bottom-nav-height\);/);
assert.match(stylesCss, /\.app\.plugin-context-nav-mode\.embedded-plugin-host-active \{[\s\S]*?padding-bottom: var\(--plugin-context-bottom-nav-height\);/);
...
assert.match(indexHtml, /app-plugin-topics-ui\.js\?v=20260603-directory-topic-back-v511/);
assert.match(serviceWorkerJs, /\/app-plugin-topics-ui\.js\?v=20260603-directory-topic-back-v511/);
assert.match(indexHtml, /app-directory-topics-ui\.js\?v=20260603-directory-topic-back-v511/);
assert.match(serviceWorkerJs, /\/app-directory-topics-ui\.js\?v=20260603-directory-topic-back-v511/);
...
assert.match(appJs, /id: "email"[\s\S]*?id: "health"[\s\S]*?id: "directory"[\s\S]*?builtinKind: "directory"/);
...
assert.match(appJs, /availablePluginTopicDefs\(\)\.filter\(\(def\) => !def\.builtinKind\)/);
...
assert.match(stylesCss, /\.plugin-app-grid \{[\s\S]*?grid-template-columns: repeat\(auto-fit, minmax\(78px, 1fr\)\)/);
assert.match(stylesCss, /\.task-list-mode \.plugin-app-grid \{[\s\S]*?grid-template-columns: repeat\(4, minmax\(0, 1fr\)\)/);
assert.match(stylesCss, /\.plugin-app-card \{[\s\S]*?min-height: 76px;[\s\S]*?background: transparent;[\s\S]*?border: 0;/);
