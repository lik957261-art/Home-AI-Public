...
- Scoped HTTP/API requests to the current account/workspace's documented Program APIs are **Allowed** when the run has the `http` toolset. Use this for authenticated manifest, bundle, validation, or writeback calls whose endpoint, credential, and scope are documented in an allowed workspace file. Do not use terminal/code for these calls unless those developer toolsets are explicitly allowed.
