...
assert.match(stylesCss, /\.composer\s*\{[^}]*grid-template-columns:\s*40px minmax\(0, 1fr\) minmax\(50px, auto\);/s);
assert.match(stylesCss, /#messageInput\s*\{[^}]*grid-column:\s*2;/s);
assert.match(stylesCss, /#sendMessage\s*\{[^}]*grid-column:\s*3;[\s\S]*?min-width:\s*58px;[\s\S]*?padding:\s*0 10px;/);
assert.match(stylesCss, /@media \(max-width: 1099px\)[\s\S]*?\.composer\s*\{[^}]*grid-template-columns:\s*40px minmax\(0, 1fr\) 52px;/);
