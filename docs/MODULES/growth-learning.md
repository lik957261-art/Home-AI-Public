...
- Model-assisted learning-plan decomposition uses a long-running model budget by default and may make one model repair pass when the first response is not strict JSON or does not match the `dailyPlans` schema. With `requireModel=true`, deterministic fallback is still not allowed: if the model times out or the repair pass also fails, the draft fails closed.
