...
assert.match(stylesCss, /\.codex-mode \.conversation,[\s\S]*?\.email-mode \.conversation,[\s\S]*?\.health-mode \.conversation,[\s\S]*?\.note-mode \.conversation\s*\{[\s\S]*padding: 0;/);
...
assert.match(stylesCss, /\.task-list-mode \.topic-plugin-dock\s*\{[\s\S]*position: fixed;[\s\S]*bottom: calc\(var\(--topic-plugin-dock-bottom\) - var\(--topic-plugin-dock-nav-overlap\)\);/);
...
assert.match(stylesCss, /\.task-list-mode \.plugin-app-strip\s*\{[\s\S]*width: 100%;/);
