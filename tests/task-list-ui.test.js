...
const CLIENT_VERSION = "20260602-topic-card-density-v461";
const appJs = [
...
assert.match(indexHtml, /<link rel="preload" href="\/styles\.css\?v=20260602-topic-card-density-v461" as="style" onload="this\.onload=null;this\.rel='stylesheet'">/);
assert.match(indexHtml, /<noscript><link rel="stylesheet" href="\/styles\.css\?v=20260602-topic-card-density-v461"><\/noscript>/);
...
assert.match(appJs, /bottomTasksMode"\)\?\.addEventListener\("click"[\s\S]*?await loadSelectedView\(\{ forceTaskListReload: true \}\)/);
assert.match(appJs, /tasksMode"\)\.addEventListener\("click"[\s\S]*?await loadSelectedView\(\{ forceTaskListReload: true \}\)/);
...
assert.match(indexHtml, /app-plugin-topics-ui\.js\?v=20260602-topic-card-density-v461/);
assert.match(serviceWorkerJs, /\/app-plugin-topics-ui\.js\?v=20260602-topic-card-density-v461/);
assert.match(indexHtml, /app-directory-topics-ui\.js\?v=20260602-topic-card-density-v461/);
assert.match(serviceWorkerJs, /\/app-directory-topics-ui\.js\?v=20260602-topic-card-density-v461/);
...
assert.match(stylesCss, /\.plugin-topic-card\.directory-special-plugin \{[\s\S]*?grid-template-rows: minmax\(102px, 1fr\);/);
...
assert.match(stylesCss, /\.plugin-topic-app-icon\.directory \{[\s\S]*?width: 88px;[\s\S]*?background: transparent url\("\/icons\/file-folder\.svg"\) center \/ contain no-repeat;[\s\S]*?box-shadow: none;/);
...
assert.match(appJs, /function scheduleDeferredDirectoryTopicRender\(threadId = ""\)/);
...
assert.match(appJs, /scheduleDeferredDirectoryTopicRender\(thread\.id\)/);
