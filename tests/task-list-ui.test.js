...
const CLIENT_VERSION = "20260609-bottom-stable-codex-back-v654";
const appJs = [
...
assert.match(indexHtml, /<link rel="preload" href="\/styles\.css\?v=20260609-bottom-stable-codex-back-v654" as="style" onload="this\.onload=null;this\.rel='stylesheet'">/);
assert.match(indexHtml, /<noscript><link rel="stylesheet" href="\/styles\.css\?v=20260609-bottom-stable-codex-back-v654"><\/noscript>/);
...
assert.match(appJs, /function requestNotePluginInstallForWorkspace\(\)/);
...
assert.match(appJs, /const listedWorkspaces = plugin\.allowWorkspaceGrant === false[\s\S]*?\: \[ownerWorkspace, \.\.\.workspaces\.filter/);
...
assert.match(appJs, /if \(pluginContextNav\) \{[\s\S]*?hiddenBottomTabs = new Set\(\["bottomChatMode", "bottomInboxMode", "bottomTodosMode", "bottomCodexMode"/);
assert.match(appJs, /const pluginContextBottomTabs = new Set\(\["bottomTasksMode", "bottomProjectsMode", pluginContextButtonId\]\)/);
...
assert.match(indexHtml, /app-plugin-topics-ui\.js\?v=20260609-bottom-stable-codex-back-v654/);
assert.match(serviceWorkerJs, /\/app-plugin-topics-ui\.js\?v=20260609-bottom-stable-codex-back-v654/);
assert.match(indexHtml, /app-directory-topics-ui\.js\?v=20260609-bottom-stable-codex-back-v654/);
assert.match(serviceWorkerJs, /\/app-directory-topics-ui\.js\?v=20260609-bottom-stable-codex-back-v654/);
...
assert.match(appJs, /dock\.hidden = !dock\.innerHTML\.trim\(\)/);
assert.match(appJs, /function preparePrimaryNavigationChange\(\) \{[\s\S]*?setTopicPluginDock\(""\)/);
...
assert.match(stylesCss, /@media \(max-width: 1099px\) \{[\s\S]*?\.task-list-mode \.topic-plugin-dock \.capability-action-menu \{[\s\S]*?position: absolute;[\s\S]*?bottom: calc\(100% \+ 10px\);[\s\S]*?z-index: 80;/);
...
assert.match(stylesCss, /\.task-list-mode \.plugin-app-strip \{[\s\S]*?width: 100%;[\s\S]*?padding: 0 max\(0px, calc\(\(100vw - 448px\) \/ 2\)\);/);
...
assert.match(stylesCss, /\.task-list-mode \.plugin-app-strip\[data-plugin-fill-count="4"\] \.plugin-app-card \{[\s\S]*?flex-basis: calc\(\(100% - var\(--plugin-app-gap\) - var\(--plugin-app-gap\) - var\(--plugin-app-gap\)\) \/ 4\);[\s\S]*?width: calc\(\(100% - var\(--plugin-app-gap\) - var\(--plugin-app-gap\) - var\(--plugin-app-gap\)\) \/ 4\);/);
assert.match(stylesCss, /\.task-list-mode \.plugin-app-strip\[data-plugin-fill-count="5"\] \.plugin-app-card \{[\s\S]*?flex-basis: calc\(\(100% - \(var\(--plugin-app-gap\) \* 4\)\) \/ 5\);[\s\S]*?width: calc\(\(100% - \(var\(--plugin-app-gap\) \* 4\)\) \/ 5\);/);
...
assert.match(stylesCss, /\.task-list-mode \.plugin-app-card \.plugin-topic-app-icon\.directory \{[\s\S]*?width: 40px;[\s\S]*?height: 40px;[\s\S]*?--directory-folder-tab-top: 9px;[\s\S]*?--directory-folder-body-top: 15px;[\s\S]*?--directory-folder-body-width: 32px;[\s\S]*?--directory-folder-body-height: 20px;[\s\S]*?border-radius: 12px;/);
