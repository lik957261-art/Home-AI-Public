...
- The next product boundary is the service data layer. `adapters/mobile-sqlite-store.js` and `scripts/migrate-json-to-sqlite.js` now provide a SQLite migration target for workspaces, access-key hashes, threads, messages, artifacts, Web Push state, shared directories, Todo, Automation, and audit events. It also supports optional SQLite-backed local Todo and Automation when `HERMES_WEB_SERVICE_STORE=sqlite` and the Todo/Automation backends are local. Production JSON thread/message files remain the rollback source until SQLite runtime mode is explicitly enabled for those state areas.
- Target shape:
