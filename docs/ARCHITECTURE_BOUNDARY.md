...
helpers and JSON store file IO, backend mode policy, workspace catalog lazy
delegation, and process HTTP lifecycle wiring addressable through CodeGraph
without loading the full runtime root.
...
- `mobile-server-runtime.js` must stay at or below 2,410 lines while it is being split further;
