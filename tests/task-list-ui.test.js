...
const CLIENT_VERSION = "20260602-topic-plugin-dock-refresh-v478";
const appJs = [
...
assert.match(indexHtml, /<link rel="preload" href="\/styles\.css\?v=20260602-topic-plugin-dock-refresh-v478" as="style" onload="this\.onload=null;this\.rel='stylesheet'">/);
assert.match(indexHtml, /<noscript><link rel="stylesheet" href="\/styles\.css\?v=20260602-topic-plugin-dock-refresh-v478"><\/noscript>/);
...
assert.match(appJs, /if \(options\?\.hard\) params\.set\("hard", "1"\)/);
...
assert.match(stylesCss, /\.wardrobe-mode \.composer,[\s\S]*?\.finance-mode \.composer,[\s\S]*?\.email-mode \.composer,[\s\S]*?\.codex-mode \.composer \{[\s\S]*?display: none;/);
...
assert.match(indexHtml, /id="bottomChatMode"[\s\S]*id="bottomInboxMode"[\s\S]*id="bottomTasksMode"[\s\S]*id="bottomProjectsMode"[\s\S]*id="bottomTodosMode"[\s\S]*id="bottomWardrobeMode"[\s\S]*id="bottomCodexMode"[\s\S]*id="bottomFinanceMode"[\s\S]*id="bottomEmailMode"/);
...
assert.match(appJs, /const hiddenBottomTabs = new Set\(\[[^\]]*"bottomPluginMode"/);
...
assert.match(appJs, /updateBottomNavVisibleCount\(\);[\s\S]*?updateTopMoreControls\(\);/);
assert.match(appJs, /"bottomEmailMode", "bottomLearningMode", "bottomAutomationMode"/);
...
assert.match(stylesCss, /\.main-back-visible\.wardrobe-plugin-host-active \.bottom-nav,[\s\S]*?\.main-back-visible\.embedded-plugin-host-active \.bottom-nav \{[\s\S]*?display: grid;/);
assert.match(indexHtml, /app-plugin-topics-ui\.js\?v=20260602-topic-plugin-dock-refresh-v478/);
assert.match(serviceWorkerJs, /\/app-plugin-topics-ui\.js\?v=20260602-topic-plugin-dock-refresh-v478/);
assert.match(indexHtml, /app-directory-topics-ui\.js\?v=20260602-topic-plugin-dock-refresh-v478/);
assert.match(serviceWorkerJs, /\/app-directory-topics-ui\.js\?v=20260602-topic-plugin-dock-refresh-v478/);
...
assert.match(stylesCss, /\.app\.task-list-mode \{[\s\S]*?padding-bottom: calc\(var\(--mobile-bottom-nav-reserved-height\) \+ var\(--topic-plugin-dock-height\)\);/);
assert.match(stylesCss, /\.task-list-mode \.topic-plugin-dock \{[\s\S]*?position: fixed;[\s\S]*?bottom: var\(--mobile-bottom-nav-reserved-height\);[\s\S]*?min-height: var\(--topic-plugin-dock-height\);[\s\S]*?background: var\(--ui-chrome\);/);
...
assert.match(appJs, /reloadForClientUpdate\(`client-version-\$\{String\(source \|\| "mismatch"\)\.slice\(0, 40\)\}`\)/);
