...
function run() {
  ...
  assert.match(permissionInstructions, /Word DOCX generation inside the current allowed roots is ordinary low-permission file delivery/);
  assert.match(permissionInstructions, /`docx_create`/);
  ...
  assert.match(fs.readFileSync(skillPath, "utf8"), /Word DOCX generation inside the current run's allowed roots is \*\*Allowed\*\*/);
  assert.match(fs.readFileSync(skillPath, "utf8"), /Use `docx_create` for explicit real \.docx document requests/);
