...
- Model-assisted learning-plan decomposition may make one model repair pass when the first response is not strict JSON or does not match the `dailyPlans` schema. With `requireModel=true`, deterministic fallback is still not allowed: if the repair pass also fails, the draft fails closed.
