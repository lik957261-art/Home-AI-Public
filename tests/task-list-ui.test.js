...
const CLIENT_VERSION = "20260605-gateway-friendly-errors-v568";
const appJs = [
...
assert.match(indexHtml, /<link rel="preload" href="\/styles\.css\?v=20260605-gateway-friendly-errors-v568" as="style" onload="this\.onload=null;this\.rel='stylesheet'">/);
assert.match(indexHtml, /<noscript><link rel="stylesheet" href="\/styles\.css\?v=20260605-gateway-friendly-errors-v568"><\/noscript>/);
...
assert.match(serverJs, /20260603-plugin-mcp-finance-health-v3/);
assert.match(gatewayRunInstructionServiceJs, /20260603-plugin-mcp-finance-health-v3/);
...
assert.match(stylesCss, /\.bottom-nav \{[\s\S]*?z-index: 40;[\s\S]*?--bottom-nav-column-min: 48px;[\s\S]*?grid-template-columns: repeat\(5, minmax\(var\(--bottom-nav-column-min\), 1fr\)\);[\s\S]*?padding: 4px env\(safe-area-inset-right\) calc\(3px \+ var\(--mobile-bottom-safe-area\)\) env\(safe-area-inset-left\);[\s\S]*?overflow-x: hidden;/);
...
assert.match(stylesCss, /\.plugin-context-nav-mode \.bottom-nav \{[\s\S]*?padding: 5px max\(22px, env\(safe-area-inset-right\)\) calc\(5px \+ var\(--mobile-bottom-safe-area\)\) max\(22px, env\(safe-area-inset-left\)\);/);
...
assert.match(indexHtml, /app-plugin-topics-ui\.js\?v=20260605-gateway-friendly-errors-v568/);
assert.match(serviceWorkerJs, /\/app-plugin-topics-ui\.js\?v=20260605-gateway-friendly-errors-v568/);
assert.match(indexHtml, /app-directory-topics-ui\.js\?v=20260605-gateway-friendly-errors-v568/);
assert.match(serviceWorkerJs, /\/app-directory-topics-ui\.js\?v=20260605-gateway-friendly-errors-v568/);
...
assert.match(appJs, /const fillCount = Math\.min\(Math\.max\(defs\.length, 1\), 5\);/);
...
assert.match(appJs, /function renderPluginAppDesktop\(defs = \[\]\)/);
assert.match(appJs, /class="capability-plugin-grid"/);
assert.match(appJs, /class="plugin-app-card capability-plugin-icon-button"/);
...
assert.doesNotMatch((appJs.match(/function renderPluginAppLauncher\(\)[\s\S]*?async function openBuiltInDirectoryPlugin\(\)/) || [""])[0], /data-plugin-topic-open-topic|data-plugin-topic-open-delivery/);
...
assert.match(appJs, /async function openBuiltInDirectoryPlugin\(\)[\s\S]*?resetDirectoryPath\(\);[\s\S]*?await loadProjects\(\);[\s\S]*?await loadDirectoryView\(\{ resetPath: true \}\);/);
...
assert.match(appJs, /setTopicPluginDock\(""\)/);
...
assert.match(stylesCss, /\.app\.task-list-mode \{[\s\S]*?padding-bottom: var\(--mobile-bottom-nav-offset-height\);/);
...
assert.match(stylesCss, /\.plugin-topic-app-icon\.directory \{[\s\S]*?width: 56px;[\s\S]*?background: transparent url\("\/icons\/file-folder\.svg"\) center \/ contain no-repeat;[\s\S]*?box-shadow: none;/);
assert.match(stylesCss, /\.plugin-topic-app-icon\.directory::after \{[\s\S]*?content: none;/);
