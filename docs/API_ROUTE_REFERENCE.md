...
| `plugin-topic-usage-api-routes.js` | `/api/plugin-topic-usage` quick-action usage preferences | access-key, workspace-scoped | Stores only bounded app/action usage counters and recency for the selected workspace. This is the source of truth for Capability Entry Hub ordering; browser `localStorage` is only a startup/offline cache. |
