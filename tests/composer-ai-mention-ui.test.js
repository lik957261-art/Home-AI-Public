...
assert.match(stylesCss, /@media \(max-width: 1099px\)[\s\S]*?\.composer\s*\{[^}]*grid-template-columns:\s*36px minmax\(0, 1fr\) 48px;[\s\S]*?padding:\s*6px max\(10px, env\(safe-area-inset-right\)\) 6px max\(8px, env\(safe-area-inset-left\)\);/);
assert.match(stylesCss, /@media \(max-width: 1099px\)[\s\S]*?#sendMessage\s*\{[^}]*min-width:\s*48px;[\s\S]*?min-height:\s*33px;[\s\S]*?padding:\s*0 6px;[\s\S]*?font-size:\s*13px;/);
