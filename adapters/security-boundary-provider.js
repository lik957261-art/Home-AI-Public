...
function permissionBoundarySkillInstructions(policy = {}) {
  ...
  return [
    ...
    "Word DOCX generation inside the current allowed roots is ordinary low-permission file delivery when the run has the file toolset; call `docx_create` for explicit real .docx document requests and return its `MEDIA:<path>` line.",
    ...
    "PowerPoint PPTX/PPTM and Excel XLSX/XLSM text extraction inside the current allowed roots is ordinary low-permission file analysis when the run has the file toolset; call `office_extract_text` when direct read_file cannot decode the Office document package.",
    "PowerPoint PPTX generation inside the current allowed roots is ordinary low-permission file delivery when the run has the file toolset; call `pptx_create` for explicit real .pptx deck requests and return its `MEDIA:<path>` line only after its compatibility validation succeeds. Use `pptx_validate` to re-check existing in-scope decks when PowerPoint compatibility is in doubt.",
    "PDF generation inside the current allowed roots is ordinary low-permission file delivery when the run has the file toolset; call `pdf_create` for explicit real .pdf document requests and return its `MEDIA:<path>` line.",
    "PDF text extraction and PDF-to-page-image rendering inside the current allowed roots are ordinary low-permission file analysis when the run has the file toolset; call `pdf_extract_text` first and `pdf_render_pages` for image/scanned PDFs instead of asking the user to export pages manually.",
    ...
    "ZIP archive listing and safe extraction inside the current allowed roots is ordinary low-permission file analysis when the run has the file toolset; call `archive_list` and `archive_extract_safe` instead of requesting shell/terminal just to unzip an in-scope archive.",
    "OCR, document-image extraction, and visual analysis of image files inside the current allowed roots are ordinary low-permission work when the run has the vision toolset; PDF containers must first go through `pdf_extract_text` or `pdf_render_pages` when those file tools are available.",
    ...
    "The current account's own wardrobe read/write/readback operations are ordinary low-permission work when the run has the wardrobe toolset and the Gateway profile binds wardrobe to that same workspace; routine wardrobe item/history writes, photo checks, and verification must use Wardrobe MCP. Do not satisfy those wardrobe operations through generic HTTP/direct Program API unless the user explicitly asks for diagnostics or manual repair.",
