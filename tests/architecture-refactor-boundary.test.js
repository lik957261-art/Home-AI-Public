...
const gatewayRunDeltaEventService = require("../adapters/gateway-run-delta-event-service");
...
const gatewayRunStartChildServiceRegistryService = require("../adapters/gateway-run-start-child-service-registry-service");
...
const mobileRuntimeGatewayStatusService = require("../adapters/mobile-runtime-gateway-status-service");
...
const nativeIosShellVersionPolicyService = require("../adapters/native-ios-shell-version-policy-service");
const nativeNotificationService = require("../adapters/native-notification-service");
const nativeSecureSecretBrokerService = require("../adapters/native-secure-secret-broker-service");
...
const systemResourceStatusService = require("../adapters/system-resource-status-service");
...
const taskCardDispatchResultService = require("../adapters/task-card-dispatch-result-service");
...
const workspaceSystemProvisioningHelperClientService = require("../adapters/workspace-system-provisioning-helper-client-service");
...
const nativeIosShellApiRoutes = require("../server-routes/native-ios-shell-api-routes");
const nativeSecureSecretApiRoutes = require("../server-routes/native-secure-secret-api-routes");
...
function testRefactorModulesExportStableContracts() {
  ...
  assert.equal(typeof gatewayRunDeltaEventService.createGatewayRunDeltaEventService, "function");
  ...
  assert.equal(typeof gatewayRunStartChildServiceRegistryService.createGatewayRunStartChildServiceRegistry, "function");
  ...
  assert.equal(typeof mobileRuntimeGatewayStatusService.createMobileRuntimeGatewayStatusService, "function");
  ...
  assert.equal(typeof nativeIosShellVersionPolicyService.createNativeIosShellVersionPolicyService, "function");
  assert.equal(typeof nativeNotificationService.createNativeNotificationService, "function");
  assert.equal(typeof nativeSecureSecretBrokerService.createNativeSecureSecretBrokerService, "function");
  ...
  assert.equal(typeof systemResourceStatusService.createSystemResourceStatusService, "function");
  ...
  assert.equal(typeof taskCardDispatchResultService.cardIdsFromTaskCardResult, "function");
  assert.equal(typeof taskCardDispatchResultService.normalizeTaskCardDispatchResult, "function");
  ...
  assert.equal(typeof nativeIosShellApiRoutes.createNativeIosShellApiRoutes, "function");
  assert.equal(typeof nativeSecureSecretApiRoutes.createNativeSecureSecretApiRoutes, "function");
...
function testServerUsesRequestContextAndSqliteCaseShareMigration() {
  ...
  const gatewayStatus = fileText("adapters/mobile-runtime-gateway-status-service.js");
  ...
  const gatewayRunDeltaEvent = fileText("adapters/gateway-run-delta-event-service.js");
  ...
  const gatewayRunStartChildRegistry = fileText("adapters/gateway-run-start-child-service-registry-service.js");
  ...
  assert.match(gatewayProvider, /createMobileRuntimeGatewayStatusService/);
  assert.match(gatewayProvider, /gatewayStatus\(\)\.getHermesStatus/);
  assert.match(gatewayStatus, /async function getHermesStatus/);
  assert.match(gatewayStatus, /gatewayPoolStatusHealthy/);
  assert.match(gatewayStatus, /fallbackError/);
  assert.doesNotMatch(gatewayProvider, /fallbackError/);
  assert.doesNotMatch(gatewayProvider, /gatewayPoolStatusHealthy\(poolStatus\)/);
  ...
  assert.match(gatewayRunEvent, /createGatewayRunDeltaEventService/);
  assert.match(gatewayRunEvent, /getDeltaEventService\(\)\.applyDelta/);
  assert.doesNotMatch(gatewayRunEvent, /^  function applyDelta\(/m);
  assert.match(gatewayRunDeltaEvent, /function applyDelta/);
  assert.match(gatewayRunDeltaEvent, /sanitizeToolsetEscalationVisibleText/);
  assert.match(gatewayRunDeltaEvent, /parseToolsetEscalationRequest/);
  assert.match(gatewayRunDeltaEvent, /message\.delta/);
  ...
  assert.doesNotMatch(gatewayRunEvent, /gateway-run-toolset-escalation-service/);
  ...
  assert.match(gatewayRunDeltaEvent, /gateway-run-toolset-escalation-service/);
  assert.match(gatewayRunOutputEvent, /gateway-run-toolset-escalation-service/);
  assert.match(gatewayRunCompletion, /gateway-run-toolset-escalation-service/);
  ...
  assert.match(gatewayRunStart, /createGatewayRunStartChildServiceRegistry/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartAssistantOptionsService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunRequestBuilderService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartEventService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartExecutionPhaseService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartPreparationService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartStreamHandoffService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartStreamOptionsService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartStateService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartTargetPhaseService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartToolsetSelectionService/);
  assert.match(gatewayRunStartChildRegistry, /createGatewayRunStartWardrobeGateService/);
  ...
  assert.doesNotMatch(gatewayRunStart, /createGatewayRunStartTargetService/);
  assert.doesNotMatch(gatewayRunStart, /targetService\.selectGatewayRunTarget/);
  assert.doesNotMatch(gatewayRunStart, /createGatewayRunStartPermissionService/);
  assert.doesNotMatch(gatewayRunStart, /permissionService\.completeModelPermissionRequest/);
  assert.doesNotMatch(gatewayRunStart, /createGatewayRunStartPluginProbeService/);
  assert.doesNotMatch(gatewayRunStart, /pluginProbeService\.runPluginCapabilityProbe/);
  ...
  assert.doesNotMatch(gatewayRunStart, /createGatewayRunStartToolsetPreflightService/);
  assert.doesNotMatch(gatewayRunStart, /toolsetPreflightService\.applyModelFirstToolsetPreflight/);
  assert.doesNotMatch(gatewayRunStart, /streamHandoffService\.startStreamHandoff/);
  assert.match(gatewayRunStartChildRegistry, /targetService\.selectGatewayRunTarget/);
  assert.match(gatewayRunStartChildRegistry, /permissionService\.completeModelPermissionRequest/);
  assert.match(gatewayRunStartChildRegistry, /pluginProbeService\.runPluginCapabilityProbe/);
  assert.match(gatewayRunStartChildRegistry, /toolsetPreflightService\.applyModelFirstToolsetPreflight/);
  assert.match(gatewayRunStartChildRegistry, /streamHandoffService\.startStreamHandoff/);
  ...
  assert.match(mobilePlatformComposition, /createNativeIosShellApiRoutes/);
  assert.match(dispatcher, /key: "nativeIosShellApiRoutes"/);
  assert.match(dispatcher, /PRE_AUTH_NATIVE_IOS_SHELL_PATHS/);
  assert.match(mobileComposition, /createNativeSecureSecretApiRoutes/);
  assert.match(dispatcher, /key: "nativeSecureSecretApiRoutes"/);
  ...
  assert.match(mobilePlatformComposition, /createOwnerSystemConsoleApiRoutes/);
  ...
  assert.match(webPushNormalization, /function scopedPushWorkspaceIds/);
  ...
  assert.match(webPushVapid, /function loadVapidConfig/);
...
function testPackageRunsArchitectureContracts() {
  ...
  assert.equal(pkg.scripts["test:install-lane"], "node scripts/run-checks.js --test-install-lane");
  assert.equal(pkg.scripts["test:all"], "node scripts/run-checks.js --test-all");
  assert.match(runner, /gitFiles\(\["ls-files", "-z", "--", pathspec\]\)/);
  assert.match(runner, /gitFiles\(\["ls-files", "--others", "--exclude-standard", "-z", "--", pathspec\]\)/);
  assert.match(runner, /INSTALL_AND_DEPLOY_LANE_TESTS/);
  assert.match(runner, /selectNodeTestFiles/);
...
function testServiceFirstArchitectureContract() {
  ...
  assert.match(doc, /mobile-runtime-gateway-provider-service\.js` must stay at or below 165\s+lines/);
  assert.match(doc, /mobile-runtime-gateway-status-service\.js` must stay at or below 50\s+lines/);
  ...
  assert.match(doc, /gateway-run-start-child-service-registry-service\.js` must stay at or below 260\s+lines/);
  ...
  assert.match(doc, /gateway-run-event-service\.js` must stay at or below 360\s+lines/);
  ...
  assert.match(doc, /mobile-api-composition\.js` must remain the top-level API aggregator/);
  ...
  assert.match(doc, /mobile-api-platform-composition\.js` must remain the public\/system\/Owner/);
  assert.match(doc, /Owner System Console and bounded\s+system resource collector services/);
