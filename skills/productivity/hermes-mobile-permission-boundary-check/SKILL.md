...
- PowerPoint PPTX generation inside the current run's allowed roots is **Allowed** when the run has the `file` toolset. Use `pptx_create` for explicit real .pptx deck requests, write only inside allowed roots, and return the generated `MEDIA:<path>` line.
