...
function permissionBoundarySkillInstructions(policy = {}) {
  ...
  return [
    ...
    "The current account's own wardrobe read/write/readback operations are ordinary low-permission work when the run has the wardrobe toolset and the Gateway profile binds wardrobe to that same workspace; use wardrobe MCP for wardrobe item/history writes and verification instead of generic HTTP when it is available.",
