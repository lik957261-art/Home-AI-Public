...
const CLIENT_VERSION = "20260603-plugin-manual-order-v522";
const appJs = [
...
assert.match(indexHtml, /<link rel="preload" href="\/styles\.css\?v=20260603-plugin-manual-order-v522" as="style" onload="this\.onload=null;this\.rel='stylesheet'">/);
assert.match(indexHtml, /<noscript><link rel="stylesheet" href="\/styles\.css\?v=20260603-plugin-manual-order-v522"><\/noscript>/);
...
assert.match(appJs, /const directoryTopicComposerOpen = Boolean\(state\.taskDirectoryFilter\?\.projectId \|\| state\.pendingTaskDirectory\?\.projectId\);/);
...
assert.match(appJs, /if \(directoryTopicComposerOpen && state\.pendingTaskDirectory\?\.projectId\) \{[\s\S]*?Start a topic for \$\{label\}\.[\s\S]*?focusComposerSoon\(\);[\s\S]*?return;/);
...
assert.match(appJs, /function isDirectoryTopicDraftActive\(\) \{[\s\S]*?Boolean\(state\.pendingTaskDirectory\?\.projectId\);/);
...
assert.match(indexHtml, /app-plugin-topics-ui\.js\?v=20260603-plugin-manual-order-v522/);
assert.match(serviceWorkerJs, /\/app-plugin-topics-ui\.js\?v=20260603-plugin-manual-order-v522/);
assert.match(indexHtml, /app-directory-topics-ui\.js\?v=20260603-plugin-manual-order-v522/);
assert.match(serviceWorkerJs, /\/app-directory-topics-ui\.js\?v=20260603-plugin-manual-order-v522/);
...
assert.match(appJs, /const directoryTopicDraftSend = state\.viewMode === "tasks"[\s\S]*?!state\.currentTaskGroupId[\s\S]*?Boolean\(state\.pendingTaskDirectory\?\.projectId\);/);
