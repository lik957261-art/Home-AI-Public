...
- 提供移动端可用的聊天、话题流、目录、看板、自动化、群聊、微信专用窗口和设置界面。
...
- 不把 Weixin/iLink polling 放回官方 Hermes Gateway 补丁中。
...
| `weixin-ingress-provider` | Weixin/iLink 事件归一化、路由、delivery id、ack 校验 |
...
### 5.8 Weixin/iLink 外部入口

Weixin/iLink 是 Hermes Mobile sidecar 边界，而不是官方 Gateway 补丁：

```mermaid
flowchart LR
  Poller["iLink / Weixin poller sidecar"]
  Ingress["POST /api/ingress/weixin/events"]
  Mobile["Hermes Mobile routing + queue"]
  Gateway["Official Hermes Gateway profile"]
  Outbox["GET /api/ingress/weixin/outbound"]
  Sender["iLink / Weixin sender sidecar"]

  Poller --> Ingress --> Mobile --> Gateway --> Mobile --> Outbox --> Sender
```

约束：

- Weixin ingress key 与浏览器 Access Key 分离。
- 同一个 Weixin 账号只能有一个 poller；启用 Mobile sidecar 前应停掉 legacy direct Gateway poller。
- 路由优先级为 explicit workspace、principalId、account/chat/user metadata、默认 workspace。
- outbound delivery 必须 ack 为 `sent`、`failed` 或 `skipped`。
- attachment-only 事件可先在 Mobile ingress 层等待后续文本指令，不直接启动无意义模型 run。
- `#` heartbeat 是 ingress 控制信号，不创建消息或模型 run。
...
- Weixin ingress：`HERMES_MOBILE_WEIXIN_INGRESS_KEY_PATH`
...
- Weixin：`/api/ingress/weixin/events`、`/api/ingress/weixin/outbound`、`/api/weixin/forward-*`
...
- Web Push 不携带秘密或裸路径。
- Weixin ingress 去重、路由、outbound delivery 和 ack 可闭环。
...
- Weixin/iLink、Kanban、Automation、外部 connector 均以 sidecar 或 adapter 形态接入。
