...
function materializeTask(program = {}, draft = {}, day = {}, task = {}, options = {}) {
  ...
  const card = {
    ...
    taskModel,
    teachingFlow: task.teachingFlow || task.taskModel?.teachingFlow || null,
    experienceSummary: task.experienceSummary || null,
    completionPolicy: task.completionPolicy || null,
    ...
    expectedDurationMinutes: task.expectedDurationMinutes || null,
    stageAssessment: task.stageAssessment || null,
    stageAssessmentCycleId: cleanString(task.stageAssessmentCycleId || task.stageAssessment?.cycleId),
    activationState: cleanString(task.activationState || task.stageAssessment?.activationState),
    activationReason: cleanString(task.activationReason || task.stageAssessment?.activationReason),
    activationSource: cleanString(task.activationSource || task.stageAssessment?.activationSource),
    cooldownUntil: cleanString(task.cooldownUntil || task.stageAssessment?.cooldownUntil),
