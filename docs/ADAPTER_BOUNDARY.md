...
- `adapters/mobile-runtime-boot-trace-service.js` owns best-effort startup trace file writes through injected filesystem, path, process, and clock dependencies. It must not grow into a logging, telemetry, lifecycle, or health-reporting module.
- `adapters/mobile-runtime-natural-language-gateway-service.js` owns runtime natural-language model-text execution through Gateway Pool target selection, streaming text aggregation, timeout flooring, and target release semantics. Natural-language prompt/schema rules stay in `adapters/natural-language-draft-service.js`; this facade must not grow into a prompt authoring, Kanban planning, or Gateway scheduling policy module.
...
- `adapters/mobile-runtime-gateway-facade-service.js` owns runtime Gateway runner, pool, profile launcher, workspace provisioning, telemetry lazy delegation, run concurrency checks, and Hermes status composition. It may attach Gateway Pool status to the single-runner status and mark the public status healthy when the single fallback runner is down but the configured pool is healthy; it must not own worker selection policy, profile launch scripts, telemetry storage, or run lifecycle streaming.
...
- `adapters/mobile-sqlite-store.js` owns the first generic SQLite service-layer schema, JSON import boundary, and optional runtime state backend. It can import/export current JSON state, access-key hash stores, shared-directory records, local Todo rows, explicit local Automation test/import rows, and Web Push state while preserving `raw_json` for compatibility. Production Automation job definitions stay in the canonical official Hermes CRON store and are projected through `adapters/automation-provider.js`.
...
- `tests/mobile-runtime-boot-trace-service.test.js` is the contract smoke for disabled startup tracing, trace line formatting, and best-effort error handling.
- `tests/mobile-runtime-natural-language-gateway-service.test.js` is the contract smoke for natural-language Gateway text delegation, stream aggregation, timeout flooring, idle release, and failed release.
...
- Weixin message polling/delivery is retired. Historical `weixin_*` workspace
  ids remain workspace identities only and must not be interpreted as an active
  polling or outbound-delivery transport.
