...
- Fresh product installs may use a local Todo store under `HERMES_WEB_DATA_DIR`.
  Automation is different: ordinary product/runtime installs must use official
  Hermes CRON through the Home AI Automation API as the canonical job store.
  Local JSON/SQLite Automation may be selected only for focused tests, local
  experiments, or explicit import/migration work, and must not silently replace a
  missing or unreadable canonical scheduler.
...
- The next product boundary is the service data layer. `adapters/mobile-sqlite-store.js` and `scripts/migrate-json-to-sqlite.js` now provide a SQLite migration target for workspaces, access-key hashes, threads, messages, artifacts, Web Push state, shared directories, Todo, explicit local Automation import/test rows, and audit events. `HERMES_WEB_SERVICE_STORE=sqlite` enables SQLite runtime state for threads/messages/artifacts/Web Push and can also back local Todo. It may back Automation only when local Automation is explicitly selected for test/import work; it is not a production mirror of official Hermes CRON. SQLite runtime still writes `state.json` snapshots after successful database writes for rollback.
- Target shape:
...
- Add process-isolation instructions for Windows, macOS, and Linux. Production should run from a clean runtime package with a low-privilege account instead of from the development checkout when OS-level source isolation is required.
