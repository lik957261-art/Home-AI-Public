...
function createGatewayRunInstructionService(options = {}) {
  ...
  function callableFunctionHintsForToolsets(toolsets = []) {
    const hintsByToolset = {
      ...
      cronjob: ["cronjob_mobile", "cronjob"],
      memory: ["memory"],
  ...
  function formatAccessPolicyInstructionSummary(policy = {}) {
    ...
    if (callableHints.length) {
      ...
      if (toolsets.includes("cronjob")) lines.push("- For Hermes Mobile automation jobs, use `cronjob_mobile` when available; raw `cronjob` may point at an empty profile-local scheduler namespace.");
    }
  ...
  function currentToolSchemaOverrideInstructions(policy = {}) {
    ...
    if (policyHasToolset(policy, "cronjob")) {
      lines.push(
        "Current tool schema override: the `cronjob` toolset is enabled for this run. For Hermes Mobile automations, prefer `cronjob_mobile` when available; it talks to the live Mobile automation bridge and scopes list/create/update/pause/resume/delete by `owner_principal_id`.",
        "Set `owner_principal_id` to the current run Principal exactly. Do not use `cronjob_mobile` to inspect or mutate another principal's jobs unless the current run is explicitly Owner-authorized for that target.",
