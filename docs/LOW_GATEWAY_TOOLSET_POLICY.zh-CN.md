...
- `file`：当前 run 授权 roots 内的文件读写，以及普通文档/归档处理能力，例如 profile-local `docx_create` 在授权 roots 内生成真实 Word `.docx` 交付文件并返回 `MEDIA:<path>`，`docx_extract_text` 对授权 roots 内 DOCX/Word OpenXML 文件做文本预览提取，`office_extract_text` 对授权 roots 内 PPTX/PPTM/XLSX/XLSM 做文本提取，`pptx_create` 在授权 roots 内生成真实 PowerPoint `.pptx` 交付文件，并且只有兼容性验证通过才返回 `MEDIA:<path>`，`pptx_validate` 对授权 roots 内既有 PPTX 做 PowerPoint 兼容性验证，`pdf_create` 在授权 roots 内生成真实 `.pdf` 交付文件并返回 `MEDIA:<path>`，`pdf_extract_text` 对授权 roots 内 PDF 文本层做提取，`pdf_render_pages` 把无文本层 PDF 渲染成受控 artifacts 页图后再交给 vision/OCR，`archive_list` / `archive_extract_safe` 对授权 roots 内 ZIP 文件做目录列出和安全解压。该能力不等同于 shell、terminal、包管理器或代码执行；PDF/Word/PPTX 生成必须写入授权 roots，不得生成伪文件或只返回 HTML/Markdown；PPTX 交付不得绕过兼容性验证；PDF 处理不得要求用户手动导出页图；ZIP 解压必须拒绝路径穿越、绝对路径、符号链接、加密项、覆盖现有文件和超限归档。
...
`allowed_toolsets` 表示 Hermes Mobile 允许该类能力；真实 callable 还取决于 Gateway profile 的插件和运行时依赖。已经产品化为 profile-local plugin 的能力，例如 `weather`、`http`、fallback `web`、DOCX 文本解析、Office 文本解析、PDF 文本提取/页图渲染、`image_gen` 编辑/擦除，应随 `scripts/configure-low-gateways.sh` 自动安装到 lowgw。`browser` 还依赖 worker runtime 中的 `agent-browser` 和 Chromium/Lightpanda/云浏览器配置；没有这些依赖时，policy 和 profile 可以开放 `browser`，但真实 schema 不会出现 `browser_*` callable。不能把这种情况描述为“权限不允许”，应描述为“运行时 browser 依赖未就绪”。
