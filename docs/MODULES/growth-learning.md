...
- Growth model calls may use either SSE or ordinary JSON responses from Gateway. Non-stream JSON responses must still be surfaced to `hermesModelText`; otherwise required-model card generation can receive an empty string and fail as invalid JSON.
