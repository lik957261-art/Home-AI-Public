...
assert.match(appJs, /function moaMentionOption\(\)/);
assert.match(appJs, /mentionText: "@MOA"/);
assert.match(appJs, /provider: "moa"/);
assert.match(appJs, /moa: true/);
...
assert.match(appJs, /CHAT_COMPOSER_MODEL_SELECTION_MODEL_ESM_PATH/);
assert.match(appJs, /importChatComposerModelSelectionModel/);
assert.match(appJs, /composerAiMentionInfoPlan/);
assert.match(appJs, /selectedComposerModelPlan/);
assert.match(appJs, /selectedComposerProviderPlan/);
...
assert.match(appJs, /ownerElevationOnceToken: state\.ownerElevationOnceToken/);
assert.match(appJs, /body\.ownerElevationOnceToken = input\.ownerElevationOnceToken/);
...
assert.match(appJs, /const ownerModelElevationRequired = Boolean\(aiMention\.ownerElevationRequired\)/);
assert.match(appJs, /const ownerModelOnceApproved = Boolean\(input\.ownerModelOnceApproved \|\| input\.chatGptProOnceApproved\)/);
assert.match(appJs, /body\.chatGptProGenerate = true/);
assert.match(appJs, /body\.requiredTool = "chatgpt_pro_generate"/);
assert.match(appJs, /body\.elevationScope = input\.chatGptProRequested \? "chatgpt_pro_generate" : "owner_high_privilege"/);
...
assert.match(appJs, /if \(input\.reasoningEffort\) body\.reasoning_effort = input\.reasoningEffort/);
assert.match(appJs, /const selectedModel = selectedComposerModel\(text\)/);
assert.match(appJs, /if \(input\.model\) body\.model = input\.model/);
assert.match(appJs, /const provider = selectedComposerProvider\(text\)/);
assert.match(appJs, /if \(input\.provider\) body\.provider = input\.provider/);
...
assert.match(appJs, /CHAT_COMPOSER_SOURCE_MODEL_ESM_PATH/);
assert.match(appJs, /selectedComposerSearchSourceInfoPlan/);
assert.match(appJs, /composerSourceControlPlan/);
assert.match(appJs, /const searchSourceFields = composerSearchSourceBodyFields\(text\)/);
assert.match(appJs, /if \(input\.searchSourceFields\) Object\.assign\(body, input\.searchSourceFields\)/);
...
assert.match(appJs, /const text = input && "value" in input \? input\.value : input\?\.innerText/);
assert.match(appJs, /input\.setRangeText\(plan\.text, plan\.selectionStart, plan\.selectionEnd, "end"\)/);
...
assert.match(upsertMessageBody, /invalidateComposerMessageProjection\(mergedMessage/);
const messageInvalidationBody = appJs.match(/function invalidateComposerMessageProjection\(message = \{\}, options = \{\}\) \{[\s\S]*?\n\}/)?.[0] || "";
assert.match(messageInvalidationBody, /assistantMessage[\s\S]*?&& typeof scheduleStreamingMessageRender === "function"[\s\S]*?&& scheduleStreamingMessageRender\(message\)/);
assert.match(messageInvalidationBody, /composerMessageActiveStatus\(message\)/);
assert.match(messageInvalidationBody, /composerMessageProjectionPlan/);
assert.match(messageInvalidationBody, /plan\.shouldScheduleRunProgress[\s\S]*?scheduleRunProgressRenderForRun\(plan\.runProgressRunId/);
