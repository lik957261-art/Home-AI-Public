...
| `native-ios-shell-api-routes.js` | iOS native shell version policy | public-safe, no private payload | `GET /api/native/ios-shell/version-policy?platform=ios&buildNumber=<build>&version=<version>` returns bounded TestFlight update policy metadata only. It is pre-auth so native can check before key unlock, defaults to minimum/latest build `35`, and never reads user/plugin content. |
...
| `ai-ops-diagnostic-api-routes.js` | diagnostic event intake, case/event inbox, and case state transitions | workspace-scoped event submission; owner for case/event management | Uses `ai-ops-diagnostic-intake-service`; event submission stores bounded metadata only, hashes thread/turn ids, strips unsafe route params, and does not accept screenshots, raw chat content, cookies, keys, launch tokens, provider payloads, or long logs. |
| `plugin-conversation-action-api-routes.js` | plugin conversation repair-request creation and Owner-triggered task-card dispatch | workspace-scoped request creation; owner for task-card dispatch | Uses `plugin-conversation-action-bridge-service`; creates Owner Action Inbox approvals from bounded host conversation requests, never auto-dispatches, and allows Owner to attach a bounded prompt only at approval time. |
| `autonomous-delivery-api-routes.js` | delivery-loop case ledger, Owner start, verification dispatch, deployment/readback dispatch, repair dispatch, Owner closure, final-report projection, return-state recording, duplicate suppression, and return-card Watchdog | workspace-scoped case create/list/detail; owner for start, verification, deployment/readback, repair, closure, Watchdog, and return recording | Uses `autonomous-delivery-coordinator-service`; case creation only opens an Owner Action Inbox start item, while task-card dispatch happens only after Owner start and only for non-high-risk slices. Stable case idempotency suppresses duplicate Owner approvals, duplicate repair/deploy/verification cards, and duplicate Web Push notifications by returning the existing case with bounded duplicate evidence. Each slice stores a bounded AI Ops required-check/evidence projection. Completed implementation/repair returns can open an Owner review item. When deployment/readback is required, Owner-triggered deployment creates a separate deployment slice and sends one deployment/readback card to the owning workspace before verification. Owner-triggered verification creates a separate verification slice and sends one audit-thread task card. Completed verification returns move the case to `verified_waiting` and create an Owner closure projection instead of recursively requesting verification again. Failed verification returns create an Owner repair projection; repair task cards are dispatched only after Owner action and route back to the original implementation workspace. Owner closure creates a bounded final-report Action Inbox delivery row with AI Ops check/evidence summaries, evidence-ledger verification status, and hash-only artifact references. Return recording accepts explicit case/slice ids, the original dispatched task-card id, or a bounded return-card event payload; return metadata may include an evidence ledger path for transient local verification, but raw paths are not persisted. The return-card Watchdog marks stale non-terminal dispatches as `return_stale` for Owner/coordinator inspection without retrying, redispatching, or closing the slice. |
| `home-ai-tts-api-routes.js` | local TTS synthesis, workspace TTS Profiles, asset metadata, audio file serving, Music demo narration batches | workspace-scoped | Uses `home-ai-tts-service`; stores metadata in SQLite and audio files under the configured TTS data directory plus a Roon watched-folder copy. TTS Profile creation stores bounded wav prompt assets and transcript metadata under the selected workspace. Routes must not expose raw secrets or use user text as file names. |
| `hermes-plugin-api-routes.js` | embedded plugin list/manifest/proxy/notifications | access-key, workspace-scoped, proxy path unauthenticated after launch | Notification events upsert summary-only `sourceType=plugin` Inbox items and send Web Push through Hermes; same-origin proxy paths do not expose raw plugin keys. The only public proxy callback exception is Music TIDAL OAuth callback with `state` and `code` or `error`; all other proxy requests stay workspace-gated. |
...
| `action-inbox-api-routes.js` | Action Inbox list/detail/manual items/state transitions, Finance ledger join approval, and plugin conversation repair approvals | workspace-scoped | Uses local SQLite `action_inbox_*` tables; Inbox items are summary/action projections, not canonical source records. Finance approval routes call the Finance review contract before updating Inbox state. Plugin conversation repair approvals dispatch only through the Owner-gated bridge. |
