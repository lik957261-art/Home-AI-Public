...
function testServerUsesRequestContextAndSqliteCaseShareMigration() {
  ...
  assert.match(server, /const normalizeLocalPath = \(\.\.\.args\) => getMobileRuntimePathAccessService\(\)\.normalizeLocalPath\(\.\.\.args\)/);
  assert.match(server, /const windowsPathToWsl = \(\.\.\.args\) => getMobileRuntimePathAccessService\(\)\.windowsPathToWsl\(\.\.\.args\)/);
  assert.match(server, /const isPathAllowed = \(\.\.\.args\) => getMobileRuntimePathAccessService\(\)\.isPathAllowed\(\.\.\.args\)/);
  assert.match(pathAccessService, /function createMobileRuntimePathAccessService/);
  assert.match(pathAccessService, /filesystemMountProvider\.normalizeLocalPath/);
  assert.match(pathAccessService, /pathPolicyProvider\.canReadForThread/);
  assert.match(pathAccessService, /securityBoundaryProvider\.isProtectedPath/);
...
function testServiceFirstArchitectureContract() {
  ...
  assert.match(doc, /1,280 lines/);
  assert.match(doc, /top-level `function` declarations in `mobile-server-runtime\.js` must stay at or below 2/);
  ...
  assert.ok(runtimeLineCount <= 1280, `mobile-server-runtime.js line budget exceeded: ${runtimeLineCount} > 1280`);
  assert.ok(runtimeTopLevelFunctionCount <= 2, `mobile-server-runtime.js top-level function budget exceeded: ${runtimeTopLevelFunctionCount} > 2`);
  ...
  assert.ok(pathAccessServiceLineCount <= 70, `mobile-runtime-path-access-service.js line budget exceeded: ${pathAccessServiceLineCount} > 70`);
