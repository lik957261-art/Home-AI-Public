...
function testGatewayConversationIdEpochForSchemaSensitiveToolsets() {
  ...
  assert.equal(
    ...
    "session_a_group_1_20260610-email-bulk-local-delete-mcp-v1_file",
  );
  assert.equal(
    service.gatewayConversationId(thread, message, { allowed_toolsets: ["memory"] }),
    "session_a_group_1_20260610-email-bulk-local-delete-mcp-v1_memory",
  );
  assert.equal(
    service.gatewayConversationId(thread, message, { allowed_toolsets: ["x_search"] }),
    "session_a_group_1_20260610-email-bulk-local-delete-mcp-v1_x_search",
  );
  assert.equal(
    service.gatewayConversationId(thread, message, { allowed_toolsets: ["vision", "wardrobe", "file"] }),
    "session_a_group_1_20260610-email-bulk-local-delete-mcp-v1_file-vision-wardrobe",
  );
}
