...
const gatewayRuntimeChildServiceRegistryJs = fs.readFileSync(path.join(repoRoot, "adapters", "gateway-runtime-child-service-registry-service.js"), "utf8");
...
const streamFirstEventServiceJs = fs.readFileSync(path.join(repoRoot, "adapters", "gateway-run-stream-first-event-service.js"), "utf8");
...
assert.match(gatewayRuntimeCompositionJs, /createGatewayRuntimeChildServiceRegistry/);
assert.match(gatewayRuntimeCompositionJs, /const lifecycleService = childServices\.lifecycleService/);
assert.match(gatewayRuntimeCompositionJs, /return getStreamService\(\)\.streamResponse/);
assert.match(gatewayRuntimeChildServiceRegistryJs, /createGatewayRunLifecycleService/);
assert.match(gatewayRuntimeChildServiceRegistryJs, /createGatewayRunStreamService/);
...
assert.match(streamServiceJs, /createGatewayRunStreamFirstEventService/);
assert.match(streamFirstEventServiceJs, /run\.model_first_byte_retrying/);
