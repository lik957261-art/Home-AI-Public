...
- The next product boundary is the service data layer. `adapters/mobile-sqlite-store.js` and `scripts/migrate-json-to-sqlite.js` now provide a SQLite migration target for workspaces, access-key hashes, threads, messages, artifacts, Web Push state, shared directories, Todo, Automation, and audit events. This is intentionally an isolated migration path first; production JSON runtime files remain the rollback source until SQLite runtime mode is explicitly enabled.
- Target shape:
