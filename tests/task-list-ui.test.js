...
const CLIENT_VERSION = "20260610-plugin-topic-claim-v686";
const appJs = [
...
assert.match(indexHtml, /<link rel="preload" href="\/styles\.css\?v=20260610-plugin-topic-claim-v686" as="style" onload="this\.onload=null;this\.rel='stylesheet'">/);
assert.match(indexHtml, /<noscript><link rel="stylesheet" href="\/styles\.css\?v=20260610-plugin-topic-claim-v686"><\/noscript>/);
...
assert.match(appJs, /\$\("todosMode"\)\.addEventListener\("click"[\s\S]*?state\.viewMode = "growth";/);
assert.match(appJs, /\$\("bottomTodosMode"\)\?\.addEventListener\("click"[\s\S]*?state\.viewMode = "growth";/);
...
assert.match(indexHtml, /app-plugin-topics-ui\.js\?v=20260610-plugin-topic-claim-v686/);
assert.match(serviceWorkerJs, /\/app-plugin-topics-ui\.js\?v=20260610-plugin-topic-claim-v686/);
assert.match(indexHtml, /app-directory-topics-ui\.js\?v=20260610-plugin-topic-claim-v686/);
assert.match(serviceWorkerJs, /\/app-directory-topics-ui\.js\?v=20260610-plugin-topic-claim-v686/);
...
assert.match(appJs, /function capabilityHubQuickActions\(defs = \[\]\)/);
...
assert.match(appJs, /if \(!count\) return;/);
...
assert.match(appJs, /conversation\.innerHTML = regularGroups\.length \|\| pluginTopicCards \|\| capabilityEntryHub \|\| directoryTopicCards/);
assert.match(appJs, /\$\{filterBanner\}\$\{pluginTopicCards\}\$\{capabilityEntryHub\}\$\{directoryTopicCards\}/);
...
assert.match(stylesCss, /\.global-plugin-dock-mode \.topic-plugin-dock\.global-plugin-dock-collapsed \{[\s\S]*?transform: translateY\(calc\(var\(--topic-plugin-dock-collapsed-offset\) - var\(--topic-plugin-dock-collapsed-safe-lift\)\)\);[\s\S]*?pointer-events: none;/);
...
assert.match(appJs, /if \(capabilityEntryHub && conversation\.scrollTop > 0 && conversation\.scrollTop <= 52\) \{[\s\S]*?conversation\.scrollTop = 0;/);
assert.match(appJs, /scheduleDeferredDirectoryTopicRender\(thread\.id, options\.restoreScrollTop, directoryTopicSignature\)/);
assert.match(appJs, /function scheduleDeferredDirectoryTopicRender\(threadId = "", restoreScrollTop = null, signature = ""\)/);
