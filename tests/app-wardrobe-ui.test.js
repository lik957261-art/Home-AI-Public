...
assert.match(wardrobeUi, /function wardrobePluginNavigationAvailable\(\)/);
assert.match(wardrobeUi, /embeddedPluginListedForWorkspace\("wardrobe"\)/);
assert.match(wardrobeUi, /refreshEmbeddedPluginList\(\)\.catch\(\(\) => \{\}\)/);
assert.match(wardrobeUi, /state\.auth\?\.isOwner && workspaceId === "owner"/);
...
assert.match(wardrobeUi, /function renderWardrobePluginFrame\(manifest, entryUrl = String\(manifest\?\.entry\?\.url \|\| ""\)\)/);
assert.match(wardrobeUi, /src="\$\{escapeHtml\(entryUrl\)\}"/);
assert.match(wardrobeUi, /renderWardrobePluginFrame\(pluginManifest, entryUrl\)/);
assert.match(wardrobeUi, /function wardrobePluginProxyEntryWorkspaceMatches\(entryUrl = "", workspaceId = ""\)/);
assert.match(wardrobeUi, /parsed\.pathname\.startsWith\("\/api\/hermes-plugins\/wardrobe\/proxy"\)/);
assert.match(wardrobeUi, /const entryWorkspaceId = parsed\.searchParams\.get\("workspaceId"\) \|\| parsed\.searchParams\.get\("workspace_id"\) \|\| ""/);
assert.match(wardrobeUi, /function wardrobePluginManifestMatchesLaunchContext\(manifest = state\.wardrobePluginManifest \|\| null, workspaceId = state\.selectedWorkspaceId \|\| "owner"\)/);
assert.match(wardrobeUi, /wardrobePluginProxyEntryWorkspaceMatches\(manifest\?\.entry\?\.url, workspaceId\)/);
...
assert.match(wardrobeUi, /state\.wardrobePluginChecked && wardrobePluginManifestMatchesLaunchContext\(state\.wardrobePluginManifest, workspaceId\)/);
