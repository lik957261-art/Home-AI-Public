...
assert.ok(startGatewayPool.includes('ln -sfn `"`$shared_memory_path`" `"`$profile_memory_path`"'));
assert.match(startGatewayPool, /memories\.profile-local-markdown-backup-\`\$\(date \+%Y%m%d%H%M%S\)/);
assert.ok(startGatewayPool.includes('find `"`$profile_memory_path`" -maxdepth 1 -type f -name \'*.md\''));
assert.ok(startGatewayPool.includes('-exec cp -n {} `"`$shared_memory_path`"/ \\;'));
assert.ok(startGatewayPool.includes('-exec cp -n {} `"`$markdown_backup_dir`"/ \\;'));
assert.match(startGatewayPool, /-delete/);
assert.ok(startGatewayPool.includes('if [ ! -e `"`$profile_memory_path`" ]; then ln -sfn `"`$shared_memory_path`" `"`$profile_memory_path`"; fi'));
assert.match(startGatewayPool, /profile memories contains non-markdown files; keeping profile-local directory/);
