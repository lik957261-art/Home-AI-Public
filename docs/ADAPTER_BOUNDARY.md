...
- Runtime configuration: Owner-managed Gateway URL and API-key file path, stored without API-key plaintext.
...
- Runtime Gateway configuration is currently in core server state through `workspace/hermes-web/runtime-config.json`; future packaging may move this behind a deployment settings provider if multiple deployment profiles are needed.
