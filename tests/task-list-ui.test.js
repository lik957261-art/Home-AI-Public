...
const CLIENT_VERSION = "20260621-android-cache-refresh-v904";
const appJs = [
...
assert.match(indexHtml, /<link rel="preload" href="\/styles\.css\?v=20260621-android-cache-refresh-v904" as="style" onload="this\.onload=null;this\.rel='stylesheet'">/);
assert.match(indexHtml, /<noscript><link rel="stylesheet" href="\/styles\.css\?v=20260621-android-cache-refresh-v904"><\/noscript>/);
...
assert.match(stylesCss, /\.wardrobe-mode \.composer,[\s\S]*?\.finance-mode \.composer,[\s\S]*?\.email-mode \.composer,[\s\S]*?\.health-mode \.composer,[\s\S]*?\.note-mode \.composer,[\s\S]*?\.growth-plugin-mode \.composer,[\s\S]*?\.moira-mode \.composer,[\s\S]*?\.music-mode \.composer,[\s\S]*?\.codex-mode \.composer \{[\s\S]*?display: none;/);
...
assert.match(appJs, /const query = new URLSearchParams\(\{\s+messageMode: "tasks",\s+messageLimit: String\(TASK_MESSAGE_INITIAL_LIMIT\),\s+\}\);/);
...
assert.match(appJs, /if \(hasAnyLoadedContent && hasLoadedLatestAssistant\) return;/);
assert.match(appJs, /messageMode: "tasks",[\s\S]*?taskGroupId,[\s\S]*?messageLimit: String\(typeof TASK_MESSAGE_INITIAL_LIMIT === "number" \? TASK_MESSAGE_INITIAL_LIMIT : 80\)/);
...
assert.match(appJs, /function renderSingleWindowChatPendingShell\(options = \{\}\) \{[\s\S]*?state\.viewMode === "single" && state\.singleWindowMode === "chat"[\s\S]*?conversation\.innerHTML = `<div class="empty-state">正在载入聊天\.\.\.<\/div>`[\s\S]*?startupPerfMark\("single-window-chat-pending-shell"/);
...
assert.match(appJs, /function scheduleSingleWindowChatPendingRecovery\(reason = "pending-shell", delayMs = 2200\) \{[\s\S]*?Number\(state\.singleWindowLoadInFlightSeq \|\| 0\)[\s\S]*?scheduleSingleWindowChatPendingRecovery\("request-in-flight", 1600\);[\s\S]*?loadSingleWindow\(\{ skipSingleWindowCache: true, pendingRecovery: true \}\)\.catch\(showError\);/);
...
assert.match(appJs, /"bottomEmailMode", "bottomHealthMode", "bottomNoteMode", "bottomGrowthMode", "bottomMoiraMode", "bottomMusicMode", "bottomLearningMode", "bottomAutomationMode"/);
...
assert.match(stylesCss, /\.app\.codex-mode #musicPluginHost,[\s\S]*?\.app\.music-mode #moiraPluginHost \{[\s\S]*?display: none !important;[\s\S]*?visibility: hidden !important;[\s\S]*?pointer-events: none !important;/);
...
assert.match(indexHtml, /app-plugin-topics-ui\.js\?v=20260621-android-cache-refresh-v904/);
assert.match(serviceWorkerJs, /\/app-plugin-topics-ui\.js\?v=20260621-android-cache-refresh-v904/);
assert.match(indexHtml, /app-directory-topics-ui\.js\?v=20260621-android-cache-refresh-v904/);
assert.match(serviceWorkerJs, /\/app-directory-topics-ui\.js\?v=20260621-android-cache-refresh-v904/);
assert.match(indexHtml, /app-voice-input-ui\.js\?v=20260621-android-cache-refresh-v904/);
assert.match(serviceWorkerJs, /\/app-voice-input-ui\.js\?v=20260621-android-cache-refresh-v904/);
assert.match(indexHtml, /app-voice-learning-ui\.js\?v=20260621-android-cache-refresh-v904/);
assert.match(serviceWorkerJs, /\/app-voice-learning-ui\.js\?v=20260621-android-cache-refresh-v904/);
...
assert.match(appJs, /typeof wirePinnedPluginBottomTabUnpin === "function"[\s\S]*?\["bottomWardrobeMode", "wardrobe"\][\s\S]*?\["bottomCodexMode", "codex-mobile"\][\s\S]*?\["bottomMoiraMode", "moira"\][\s\S]*?\["bottomMusicMode", "music"\][\s\S]*?wirePinnedPluginBottomTabUnpin\(\$\(buttonId\), pluginId\)/);
...
assert.match(appJs, /function pluginTopicChatEntryStillCurrent\(pluginId, groupId, entrySeq\)[\s\S]*?state\.pluginContextNavPluginId === pluginId[\s\S]*?state\.primaryNavigationSeq/);
assert.match(appJs, /function schedulePluginTopicCachedMessageRender\(def, thread, entrySeq\)[\s\S]*?requestAnimationFrame[\s\S]*?window\.setTimeout\(run, 0\)/);
assert.match(appJs, /function renderPluginTopicChatImmediateShell\(def\)/);
assert.match(appJs, /function renderPluginTopicChatImmediateShell\(def\)[\s\S]*?const thread = pluginTopicImmediateThreadForDef\(def\)[\s\S]*?state\.currentThread = thread[\s\S]*?renderPluginTopicChatPendingShell\(def, \{ cached: true \}\)[\s\S]*?schedulePluginTopicCachedMessageRender\(def, thread, entrySeq\)/);
