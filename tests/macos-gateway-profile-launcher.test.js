...
assert.match(script, /sudo_prefix = \[\] if os\.geteuid\(\) == 0 else \["\/usr\/bin\/sudo", "-n"\]/);
assert.match(script, /cmd = sudo_prefix \+ \["\/bin\/launchctl", "kickstart"\]/);
assert.match(script, /cmd = sudo_prefix \+ \["\/bin\/launchctl", "kill", "SIGTERM"/);
